...
def autoregOFI(OFI, df, param=100):
    ...
    for i in range(int(len(OFI) / param)):
        reg = LinearRegression().fit(
            [[1, i] for i in OFI[i * param : (i + 1) * param].to_numpy()],
            df["MidReturn"][i * param : (i + 1) * param].fillna(0),
        )
        toReturn[(i + 1) * param : (i + 2) * param] = (
            reg.coef_[1] * OFI[(i + 1) * param : (i + 2) * param]
        )
    return toReturn
